...
#if defined(NDEBUG)
# error "Privcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa401bc5f4bc7da11aeeee8665a6f1c94c600e83e86c984d3545c54044aebefef");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Privcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Privcoin Signed Message:\n";
...
{
    // Privcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Privcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Privcoin: 1 days
static const int64 nTargetSpacing = 2.5 * 60; // Privcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Privcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Privcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6abf300743c4c16af1b6354e4236a814947e6f4aea76df392d692d983c78929c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Declaration of Independence Approved by Congress July 4 1776";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048c4ad1d1045ae623561a98c62f09de9cf64468444854a18e38108e4305753ddfcb17778fa727cd559c82dc7ee7850322216fb168362fe860eb669d3ae86db834") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1568252400;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084642303;

        if (fTestNet)
        {
            block.nTime    = 1568252378;
            block.nNonce   = 385907122;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x3affa7a1a01c3330dbd222e8b970f44e2314a84de302457a0fa98d3217665b3a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc5, 0xbc, 0xda }; // Privcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PrivcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PrivcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PrivcoinMiner(CWallet *pwallet)
{
    printf("PrivcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("privcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PrivcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PrivcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PrivcoinMiner, pwallet));
}
